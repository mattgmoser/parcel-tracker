...
import { advanceParcel, createParcel, getParcel, quote, validateNewParcel, ValidationError } from "./parcels";
...
export const server = createServer((req, res) => {
  ...
  if (req.method === "POST" && parts[0] === "parcels" && parts.length === 1) {
    ...
    req.on("end", () => {
      ...
      try {
        const validated = validateNewParcel(input);
        const parcel = createParcel(validated);
